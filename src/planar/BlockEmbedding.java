...
/**
 * The embedding for a single block (biconnected subgraph) of a 
 * molecular graph, as a set of faces and a combinatorial map of the vertices.
 * 
 * @author maclean
 * 
 */
public class BlockEmbedding {
    ...
    /**
     * The block that this embedding is of... XXX : should this be passed to the
     * constructor?
     */
    private Block block;
    ...
    public BlockEmbedding(IAtomContainer atomContainer) {
    ...
    /**
     * Embed a circuit, returning it as a face. The edges of the face are in
     * cyclic order. The vertices are added to the combinatorial map.
     * 
     * @param circuit
     * @return
     */
    ...
    private void embedCircuit(Block circuit, Vertex previous, Vertex current,
            Vertex start, Face face) {
...
//        Vertex end = rev.getVertex(rev.vsize() - 1);
    ...
    public Map<Bridge, List<Face>> getBridgeFaceCounts(List<Bridge> bridges) {
        Map<Bridge, List<Face>> drawableFaces = new HashMap<Bridge, List<Face>>();
        for (Bridge bridge : bridges) {
            drawableFaces.put(bridge, getDrawableFaces(bridge));
        }
