...
/**
 * The embedding of an entire molecule graph, composed of a set of blocks - or
 * biconnected components - and trees connecting these blocks, or connected to
 * them.
 * 
 * @author maclean
 * 
 */
public class AtomContainerEmbedding {
    ...
    public AtomContainerEmbedding() {
        blockEmbeddings = new ArrayList<BlockEmbedding>();
        trees = new ArrayList<Block>();
    }

    public AtomContainerEmbedding(List<Block> trees) {
        blockEmbeddings = new ArrayList<BlockEmbedding>();
        this.trees = trees;
    ...
    public Block getPartGraph() {
        Block partGraph = new Block();
        ...
        for (int blockIndexI = 0; blockIndexI < b; blockIndexI++) {
            ...
            for (int blockIndexJ = blockIndexI + 1; blockIndexJ < b; blockIndexJ++) {
                ...
                if (blockI.connectedTo(blockJ)) {
                    partGraph.add(blockIndexI, blockIndexJ);
                }
            }
            ...
            for (int treeIndex = 0; treeIndex < t; treeIndex++) {
                ...
                if (blockI.connectedTo(tree)) {
                    ...
                    partGraph.add(blockIndexI, treeIndex + b);
                }
            }
        }
        return partGraph;
