...
public class SpanningTree extends GraphObject {
    ...
    private int dfs(int index, int[] depthList, int depth, GraphObject tree,
    ...
    public Path getPath(int vIdxI, int vIdxJ) {
        ...
        // construct the path by iterating from vj->vi, following only
        // decreasing labels
        path.add(vJ);
        Vertex prev = vJ;
        ...
        while (!prev.equals(vI)) {
            ...
            if (current == null) {
                System.out.println("ERROR " + path + " " + vI + " " + vJ + " "
                        + tree + " " + Arrays.toString(labels));
            }
            prev = current;
        }
