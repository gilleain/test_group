...
/**
 * Embed a molecule in the plane (if possible) by dividing it into parts - 
 * blocks (biconnected components) and trees - and then embedding the parts 
 * separately.
 * 
 * @author maclean
 * 
 */
public class AtomContainerEmbedder {

    public static AtomContainerEmbedding embed(IAtomContainer atomContainer) {
        ...
        AtomContainerEmbedding graphEmbedding = new AtomContainerEmbedding(treeBlocks);
        for (Block cycleBlock : cycleBlocks) {
            // System.out.println("embedding " + cycleBlock);
            graphEmbedding.addBlockEmbedding(
                    PlanarBlockEmbedder.embed(cycleBlock, atomContainer));
        }
    ...
    private static Block dfsEdge(int current, List<Edge> edges, Block tree,
            BitSet visited) {
