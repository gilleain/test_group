...
public class TreeCenterFinder {
	...
	public static List<Integer> findCenter(Layoutable tree) {
//	    System.out.println(tree.esize() + "\t" + tree.vsize());
	    if (tree.esize() > tree.vsize() - 1) {
	        return new ArrayList<Integer>();
	    }
		Map<Integer, List<Integer>> adjMat = tree.getConnectionTable();
	    ...
	    if (adjMat.size() <= 2) {
    		...
    		for (Integer x : toRemove) {
    			...
    			for (Integer v : adjMat.keySet()) {
    			    ...
    			    // careful! - have to use integer here, not int
    			    partners.remove(x); 
    			}
    		}
    		prune(adjMat);
    ...
    public static Integer findUniqueCenter(Layoutable tree) {
    ...
    private static int getHeight(Layoutable tree, int root, int partner) {
        int height = 1;
    ...
    private static int getHeight(Layoutable tree, int root, BitSet visited) {
        visited.set(root);
        int height = 1;
        for (int child : tree.getConnected(root)) {
