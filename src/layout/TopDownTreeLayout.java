...
public class TopDownTreeLayout implements SimpleLayout {
    ...
    public Representation layout(Layoutable tree, Rectangle2D canvas) {
        ...
        int[] depthList = new int[tree.vsize()];
    ...
    public void layoutTree(Layoutable tree, int root, int leafCount, int maxDepth, int[] depthList) {
        this.xSep = width / (leafCount + 1);
        this.ySep = height / (maxDepth + 1);
        layout(tree, root, -1, depthList, new BitSet());
    }
    
    public double layout(Layoutable tree, int root, int parent, int[] depthList, BitSet visited) {
        visited.set(root);
...
//            if (visited.get(child)) {
    ...
    private int calculateDepth(Layoutable tree, int root, int[] depthList, int depth) {
    ...
    private int getWidth(Layoutable tree, int root) {
        BitSet visited = new BitSet();
        return getWidth(tree, root, visited);
    }
    
    private int getWidth(Layoutable tree, int root, BitSet visited) {
        visited.set(root);
