...
public class Representation {
	...
	public String toString() {
	    ...
	    for (Vertex v : points.keySet()) {
	        sb.append(v).append("\t").append(p2s(points.get(v))).append("\n");
	    }
	    for (Edge edge : lines.keySet()) {
            Line2D line = lines.get(edge);
	        sb.append(String.format("%s %s %s\n", edge, p2s(line.getP1()), p2s(line.getP2())));
	    }
	    return sb.toString();
	}
	
	private String p2s(Point2D point) {
	    return String.format("(%2.2f, %2.2f)", point.getX(), point.getY());
	}
    ...
    public List<Edge> getEdges() {
        return new ArrayList<Edge>(lines.keySet());
    }
    
    public Line2D getLine(Edge edge) {
