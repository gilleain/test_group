...
public class GraphLayout {
    ...
    public Representation layout(AtomContainerEmbedding embedding, Rectangle2D canvas) {
        Representation rep = new Representation();
        Block partTree = embedding.getPartGraph();
...
//                           " BT= " + partTree);
    ...
    private void layout(AtomContainerEmbedding embedding, Representation rep,
                        Block partTree, int partIndex, int parentIndex, 
        ...
        if (isTreePart) {
            ...
            if (articulationVertex == null) {
                ...
                rep.add(layout.layout(part, new Rectangle2D.Double(x, y, 1, 1)));
            } else {
                ...
                layout.layout(part, articulationVertex, pA, pB, rep);
            }
        } else {
...
//            rep.addPoint(new Vertex(50), nextCenter);
    ...
    private Point2D makeNextCenterForBlockToBlock(
            int partIndex, int nextPartIndex, 
            AtomContainerEmbedding embedding, 
            Vertex articulationVertex, Representation rep) {
    ...
    private Point2D makeNextCenterForTreeToBlock(
            Block part, int nextPartIndex, AtomContainerEmbedding embedding, 
            Vertex articulationVertex, Representation rep) {
    ...
    private double getRadiusForBlock(int blockIndex, AtomContainerEmbedding embedding) {
